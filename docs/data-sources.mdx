...
### CorridorRisk.io (maritime corridor risk API)

**CorridorRisk.io** provides real-time maritime corridor risk intelligence: composite scores, vessel traffic, conflict-linked events, and AI-generated briefs. It is a separate platform from World Monitor; use its API when you need corridor-level risk for TMS, ERP, or insurance workflows.

| Item | Details |
| ---- | ------- |
| **Product** | [CORRIDORRISK.io](https://corridorrisk.io) — Maritime Intelligence Platform |
| **Base URL** | `https://corridorrisk.io/api` |
| **Auth (beta)** | Open during beta; production will use API keys — contact [hello@corridorrisk.io](mailto:hello@corridorrisk.io) for early access |
| **Rate limit (beta)** | Soft limit ~100 requests/minute per IP; higher throughput on request |

**Corridor IDs** (used across all endpoints):
...
**Response fields** (per corridor object): `id`, `name`, `score` (0–100 composite), `vessel_count`, `disruption_pct` (vs baseline), `incident_count_7d`, `incident_count_30d`, `risk_report` (`situation`, `impact`, `key_incidents`, `outlook`, `action`), `computed_at`.

**Scoring methodology** (hybrid model: AIS + GDELT; GDELT-only fallback when AIS is unavailable):

| Component | Weight | Source | Role |
| --------- | ------ | ------ | ---- |
...
| Conflict composition | 20% | GDELT | Share of material conflict vs verbal tension |
| Trend | 15% | GDELT | Week-over-week change in event frequency |

**Data & AI** (per vendor): GDELT + Datalastic AIS; AI (Claude); weather via Open-Meteo.
