...
import { type TelegramWsMessage, useTelegramFeed } from '@/hooks/useTelegramFeed';
...
function renderPulseRow(m: TelegramWsMessage, i: number) {
  ...
  const body = h('div', { className: 'telegram-pulse-body' },
...
export function TelegramIntelLiveStrip() {
  ...
  return h('div', { className: 'telegram-pulse-live' },
    ...
    messages.length === 0
      ...
      : h('div', { className: 'telegram-pulse-live-list' }, ...messages.map(renderPulseRow)),
  );
}
